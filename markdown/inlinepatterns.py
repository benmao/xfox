...
from util.base import replace_latex
import logging
"""
...
GIST_LINK = r'<(http://gist.github.com/[0-9]+.js)>' #<http://gist.github.com/504542.js>
LATEX = r'\$\$(.+?)\$\$' # $$\sqrt{a^2+b^2}$$
MENTION = r'(?<=@)([a-zA-Z0-9]{3,16}\.?)'
...
class GistScriptPattern(Pattern):
    """ Return a gist  js script ('http://gist.github.com/504542.js')."""
    def handleMatch(self,m):
        el = markdown.etree.Element("script")
...
class LatexPattern(Pattern):
    '''
    Return a Latex image ('$$\sqrt{a^2+b^2}$$')
    '''
    def handleMatch(self,m):
        latex_str =m.group(2).strip()
        if not latex_str:
            return 
        el = markdown.etree.Element('img')
        x = replace_latex(latex_str)
        el.set('src','/g/latex/%s/' % x)
        el.set("alt","Latex")
        return el
            
class MentionPattern(Pattern):
    '''
    Return a user mention('@ben')
    '''
    def handleMatch(self,m):
        user = m.group(2)
        if user.endswith('.'):
